...
class AutomationTask:
    def __init__(self):
        ...
        self.TABELA_FQN = "datalab-pagamentos.ADMINISTRACAO_CELULA_PYTHON.doki_carteira"
        
        self.SAFE_SCHEMA = [
            ("carteira_contabil", "Carteira Contábil"),
            ("tipo", "Tipo"),
            ("movimento", "Movimento"),
            ("descricao", "Descrição"),
            ("valor", "Valor"),
            ("saldo_credor", "Saldo Credor"),
            ("saldo_financiado", "Saldo Financiado"),
            ("saldo_corrente", "Saldo Corrente"),
            ("saldo_futuro_parcelado_sem_juros", "Saldo Futuro Parcelado Sem Juros"),
            ("saldo_futuro_parcelado_com_juros", "Saldo Futuro Parcelado Com Juros"),
            ("saldo_futuro_juros", "Saldo Futuro Juros"),
            ("saldo_creliq", "Saldo Creliq"),
            ("saldo_acordo", "Saldo Acordo"),
            ("saldo_perda", "Saldo Perda"),
            ("saldo_transitorio_lojista", "Saldo Transitorio Lojista"),
            ("saldo_lojista_credor", "Saldo Lojista Credor"),
            ("saldo_lojista_devedor", "Saldo Lojista Devedor"),
            ("saldo_pagamentos_rejeitados", "Saldo Pagamentos Rejeitados"),
            ("saldo_em_analise", "Saldo Em Análise"),
            ("saldo_creliq_compulsorio", "Saldo Creliq Compulsório"),
            ("saldo_perda_compulsorio", "Saldo Perda Compulsório"),
            ("saldo_parcelamento_atraso", "Saldo Parcelamento Atraso"),
            ("saldo_juros_parcelamento_atraso", "Saldo Juros Parcelamento Atraso"),
            ("saldo_futuro_saque", "Saldo Futuro Saque"),
            ("saldo_futuro_juros_saque", "Saldo Futuro Juros Saque"),
            ("saldo_futuro_parcelamento_fatura", "Saldo Futuro Parcelamento Fatura"),
            ("saldo_futuro_juros_parcelamento_fatura", "Saldo Futuro Juros Parcelamento Fatura"),
            ("saldo_credor_pre_pago", "Saldo Credor Pre Pago"),
            ("saldo_recuperar_pre_pago", "Saldo Recuperar Pre Pago"),
        ]
    ...
    def _baixar_relatorio(self, page, data: date):
        try:
            url_auth = f"https://{self.DOCK_USER}:{self.DOCK_PASS}@10.19.254.121/reports/report/C6Bank/Controle%20Contábil/Movimentação%20Carteira%20Diario%20-%20Emissor"
            LOGGER.info(f"Navegando: {data}")
            page.goto(url_auth, timeout=60000)
            
            frame = page.frame_locator('iframe[src*="ReportViewer.aspx"]')
            frame.locator('[id$="ctl03_txtValue"]').fill(data.strftime("%d/%m/%Y"))
            ...
            dl = dl_info.value
            dest = TEMP_DIR / f"CARTEIRA_{data.strftime('%Y.%m.%d')}.xlsx"
            dl.save_as(str(dest))
    ...
    def _tratar_dataframe(self, caminho: Path) -> pd.DataFrame:
        try:
            ...
            for i in range(min(50, len(df_raw))):
                ...
                if "carteira contabil" in row and "saldo credor" in row:
                    hdr = i; break
